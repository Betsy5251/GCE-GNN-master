...
class GlobalAggregator(nn.Module):
    ...
    def forward(self, self_vectors, neighbor_vector, batch_size, masks, neighbor_weight, pos_weight, extra_vector=None):
        ...
        for i in range(len(self_vectors)):
            if extra_vector is not None:
                alpha = torch.matmul(torch.cat([extra_vector.unsqueeze(2).repeat(1, 1,
                                                                                 neighbor_vector[(i + 1) * 3 - 3].shape[
                                                                                     2], 1) * neighbor_vector[
                                                    (i + 1) * 3 - 3], neighbor_weight[(i + 1) * 3 - 3].unsqueeze(-1),
                                                pos_weight[(i + 1) * 3 - 3]], -1),
                                     self.w_11).squeeze(-1)
                alpha = F.leaky_relu(alpha, negative_slope=0.2)
                ...
                # alpha = torch.matmul(torch.cat([extra_vector.unsqueeze(2).repeat(1, 1,
                #                                                                  neighbor_vector.shape[
                #                                                                      2], 1) * neighbor_vector, neighbor_weight.unsqueeze(-1),
                #                                 pos_weight], -1),
                #                      self.w_11).squeeze(-1)
                # alpha = F.leaky_relu(alpha, negative_slope=0.2)
                # alpha = torch.matmul(alpha, self.w_21).squeeze(-1)
                # alpha = torch.softmax(alpha, -1).unsqueeze(-1)
                # neighbor_vector_in = torch.sum(alpha * neighbor_vector, dim=-2)


                beta = torch.matmul(
                    torch.cat([extra_vector.unsqueeze(2).repeat(1, 1, neighbor_vector[(i + 1) * 3 - 2].shape[2], 1) *
                               neighbor_vector[(i + 1) * 3 - 2], neighbor_weight[(i + 1) * 3 - 2].unsqueeze(-1),
                               pos_weight[(i + 1) * 3 - 2]], -1),
                    self.w_12).squeeze(-1)
                ...
                thea = torch.matmul(
                    torch.cat([extra_vector.unsqueeze(2).repeat(1, 1, neighbor_vector[(i + 1) * 3 - 1].shape[2], 1) *
                               neighbor_vector[(i + 1) * 3 - 1], neighbor_weight[(i + 1) * 3 - 1].unsqueeze(-1),
                               pos_weight[(i + 1) * 3 - 1]], -1),
                    self.w_13).squeeze(-1)
